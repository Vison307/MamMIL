...
class DirectMamba(nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        d_model,
        n_directions=1,
        ...
        for _ in range(self.n_ssms):
            # D "skip" parameter
            D = nn.Parameter(torch.ones(self.d_ssm if self.D_has_hdim else self.nheads, device=device))
    ...
    def forward(self, u, order_index_list=None, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        """
        ...
        order_index_list: list of indices to reorder the sequence before processing. If None, no reordering.
        """
        ...
        # A_b = -torch.exp(self.A_log_b.float())  # (nheads) or (d_inner, d_state)
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            for i in range(self.n_ssms):
                ...
                # ----> Get conv1d, dt_bias, A, D for each direction
                ...
                out = mamba_split_conv1d_scan_combined(
                    ...
                    dt_bias,
                    A,
                    D=rearrange(D, "(h p) -> h p", p=self.headdim) if self.D_has_hdim else D,
...
if __name__ == '__main__':
    model = DirectMamba(d_model=1024).cuda()